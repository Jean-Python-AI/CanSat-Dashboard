  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4 w-full h-full">
      ...
      <div className="px-4 py-4 md:py-8 flex flex-col justify-center items-end h-full flex-col gap-2">
        <h1 className="text-end font-bold text-2xl md:text-3xl">
          Hello,{isLive ? ' ' : ''}
          <br />
          the Dashboard {isLive ? 'is' : 'was'}
        </h1>
        <div className={`flex flex-row items-center gap-2 py-1 pl-4 pr-3 ${isLive ? 'bg-green-800' : 'bg-gray-800'} rounded-3xl border-2 ${isLive ? 'border-green-700' : 'border-gray-700'}`}>
          {isLive ? (
            <>
              <h1 className="font-bold text-green-200 text-xl">live</h1>
              <div className="w-3 h-3 rounded-xl bg-green-500 animate-pulse" />
            </>
          ) : (
            <>
              <h1 className="font-bold text-gray-200 text-xl">recorded</h1>
      ...
      <div className="bg-zinc-800 rounded-2xl px-4 pt-4 pb-4 flex flex-col">
        <div className='w-full flex flex-rows justify-between'>
          ...
          <p className='text-xl font-bold text-zinc-300'>last altitude</p>
        </div>
        <div className="w-full flex flex-row items-end justify-end gap-2">
          <div className="flex flex-row items-start justify-end gap-1">
            <h1 className="text-6xl xl:text-4xl font-bold">{altitude.toFixed(1)}</h1>
      ...
      <div className="bg-zinc-800 rounded-2xl px-4 py-4 flex flex-col">
        <div className='w-full flex flex-rows justify-between'>
          <h1 className="text-ms font-bold text-zinc-400">pressure</h1>
          <p className='text-xl font-bold text-zinc-300'>last pressure</p>
        </div>
        <div className="w-full flex flex-row items-end justify-end gap-2">
          <div className="flex flex-row items-start justify-end gap-1">
            <h1 className="text-6xl xl:text-4xl font-bold">{pressure.toFixed(1)}</h1>
      ...
      <div className="bg-zinc-800 rounded-xl px-4 py-4 flex flex-col">
        <div className='w-full flex flex-rows justify-between'>
          <h1 className="text-ms font-bold text-zinc-400">temperature</h1>
          <p className='text-xl font-bold text-zinc-300'>last temperature</p>
        </div>
        <div className="w-full flex flex-row items-end justify-end gap-2">
          <div className="flex flex-row items-start justify-end gap-1">
            <h1 className="text-6xl xl:text-4xl font-bold">{temperature.toFixed(1)}</h1>
